...
error_vect = Parallel(n_jobs=-1, backend='threading', verbose=10)(
    delayed(detectError)(row.split(' '), bigram_list) for idx, row in data_train_wth_duplicated.label.iteritems())
data_train_wth_duplicated['error'] = error_vect
data_train_wth_duplicated['diff']  = data_train_wth_duplicated.error.apply(lambda x: x.get(x.keys()[0])[1] - x.get(x.keys()[0])[2] if x != {} else 0)
data_train_wth_duplicated[data_train_wth_duplicated.error != {}].to_csv('/home/faris/Desktop/outwith_treated6.csv')
'''

"""
import numpy as np
nb_chunks = 4
chunks_id = np.array_split(data_train_wth_duplicated.index.tolist(),2)
index, error_vect, replace_vect = applyDetectError(data_train_wth_duplicated)
pool = Pool()
rez = pool.apply_async(applyDetectError, args = data_train_wth_duplicated.loc[chunks_id[0]])  # evaluate "solve1(A)" asynchronously
index1, error_vect0, replace_vect0 = pool.apply_async(applyDetectError, data_train_wth_duplicated.loc[chunks_id[1]])
answer1 = rez.get(timeout=10)
